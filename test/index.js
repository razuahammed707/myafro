...
const getUserData = (id) => {
    ...
    const uniqueUser = data.map((user) => user.userId).filter((val, index, self) => self.indexOf(val) === index)
    const testingData = data.map((user) => {
      if(uniqueUser.filter(i => i === user.userId)){
        return i
      }
    })
